...
class TestFmtLabels:
    ...
    def test_fmt_labels_polars(self):
        x = pl.DataFrame(
            data={
                "source": "1",
                "sex": "2",
...
class TestGetParticipantTable:
    def test_get_ppt_table(self):
...
class TestGetParticipantTable:
    def test_get_apt_table(self):
        df = utils.get_apt_table(conftest.RECORDS)

        assert isinstance(df, pl.DataFrame)

    def test_get_apt_table_study(self):
    ...
    def test_get_apt_table_id(self, ppt_id: str | None = None):
        if ppt_id is None:
...
class TestGetQuestionnaireTable:
    def test_get_que_table(self):
        df = utils.get_que_table(conftest.RECORDS)

        assert isinstance(df, pl.DataFrame)

    def test_get_que_table_id(self, ppt_id: str | list[str] | None = None):
        if ppt_id is None:
            ppt_id = choice(list(conftest.RECORDS.participants.records.keys()))
    ...
    def test_get_que_table_id_list(
        self, que_id: str | list[str] | None = None, k: int = 100
    ):
        if que_id is None:
...
class TestInDataDict:
    def test_is_in_data_dict(self):
        """Test is_in_datadict."""
        ...
        assert utils.is_in_data_dict(
            "appointment_study", dd, ["mop_newborns_1_nirs"]
        ) == ["mop_newborns_1_nirs"]
    ...
    def test_get_weekly_apts(self):
        ...
        assert isinstance(
            utils.get_weekly_apts(
                records=conftest.RECORDS, study="mop_newborns_1_nirs"
            ),
            int,
        )
        assert isinstance(
